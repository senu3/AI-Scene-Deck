...
| **Asset Metadata** | `AssetMetadata` は attached audio/offset/displayTime/analysis を保持。 | **含む:** `attachedAudioId` / `attachedAudioOffset` / `displayTime`。 | **更新:** `attachAudio` / `detachAudio` / `updateAudioAnalysis`。 | `AssetMetadata`、`metadataStore.ts` |
