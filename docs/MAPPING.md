...
| アセット     | `Asset`（path/type/vaultRelativePath 等）。 | `assetPath` の同期/解決/インポート系ユーティリティ。                         | `Sidebar`（閲覧/フォルダ追加）、`CutCard`/`PreviewModal`（表示/再生）。         |
...
| 同期       | `AssetIndex` / `VaultImportResult` 等。   | `assetPath` ユーティリティ + ElectronAPIの vault sync operations。 | 明示的な専用UIは未確認（主要フローは `StartupModal`/`Sidebar`/`Storyline` 経由）。 |
